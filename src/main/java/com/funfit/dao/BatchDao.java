...
public class BatchDao {
	...
	PreparedStatement pstmt = con.prepareStatement("insert into batch(typeofbatch,time,instructor) values(?,?,?)");
	pstmt.setString(1, batch.getTypeofbatch());
	pstmt.setString(2, batch.getTime());
	pstmt.setString(3, batch.getInstructor());
	return pstmt.executeUpdate();
		...
		while(rs.next()) {
			...
			b.setInstructor(rs.getString(4));
			listOfBatch.add(b);
			}
			} catch (Exception e) {
				System.err.println(e);
